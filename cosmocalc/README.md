...
```
每项独立返回 `ok` + `result` 或结构化 `error`，单项失败不影响其他项。每项可选 `"type"`：`"distance"`（默认）或 `"redshift"`，选择该项执行哪种核算。

**批量里的每一条核算都会作为独立的历史记录落库**，记录类型与单条接口一致（距离核算记为 `distance`，红移核算记为 `redshift`），失败项同样各自落库并携带错误信息，因此批量的每一项都能按类型在历史中单独查到。

### `GET /api/v1/history?type=distance&limit=50&offset=0` — 历史查询

按类型（`redshift`/`distance`）过滤，按时间倒序返回持久化的请求与结果；批量提交的每条谱线以其各自的核算类型入库，与逐条提交的记录同等可查。
...
- 批量每项独立落库：按核算类型（`redshift`/`distance`）各自可查，失败项带错误信息落库，历史条数恰等于核算次数
- 历史持久化：请求落库、按类型过滤、倒序
- 并发 64 路请求结果互不串扰、历史记录不重不漏；并发批量与单条混发时历史条数恰等于核算总次数
